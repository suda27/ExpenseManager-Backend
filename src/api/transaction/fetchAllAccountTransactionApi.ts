...
export class fetchAllAccountTransactionApi {
  /*
  ...
  */
  static async handler(event: APIGatewayEvent): Promise<APIGatewayProxyResult> {
    return new Promise(async (resolve, reject) => {
      try {
        const userAccountDetails: UserAccount = JSON.parse(event.body);
        logger.info(
          "At the fetchAllAccountTransactionApi ",
          userAccountDetails
        );
        const response = await transactionService.fetchAllTransactionOfAccount(
          userAccountDetails
        );
        resolve(
          formatJSONResponse(
...
export const handler = fetchAllAccountTransactionApi.handler;
